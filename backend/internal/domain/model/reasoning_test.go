...
func TestSupportedReasoningEffortsPerModel(t *testing.T) {
	tests := []struct {
		...
		{model: "Web/grok-4.6", want: []string{"low", "medium", "high", "xhigh"}},
	...
	if !SupportsReasoningEffortForProvider(account.ProviderBuild, "grok-4.6", "xhigh") ||
		!SupportsReasoningEffortForProvider(account.ProviderWeb, "grok-4.6", "xhigh") {
		t.Fatal("grok-4.6 xhigh must remain available through Build and Web")
	}
	...
	// Console 渠道已移除：固定推理模型特判不再存在，推理能力目录对所有渠道一致。
	if !SupportsReasoningForProvider(account.ProviderBuild, "grok-4.20-0309-reasoning") {
		t.Fatal("grok-4.20 reasoning must still advertise reasoning summaries")
	}
}
...
func TestReasoningAliasPublicIDs(t *testing.T) {
	...
	if got := ReasoningAliasPublicIDsForProvider(account.ProviderBuild, "grok-4.20-0309-reasoning"); len(got) != 3 {
		t.Fatalf("grok-4.20 Build aliases = %#v", got)
	}
	if got := ReasoningAliasPublicIDsForProvider(account.ProviderWeb, "grok-4.6"); len(got) != 4 || got[3] != "grok-4.6-xhigh" {
		t.Fatalf("Web grok-4.6 aliases = %#v", got)
	}
}
