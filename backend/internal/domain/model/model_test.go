...
func TestNormalizePublicIDUsesStableProviderNamespace(t *testing.T) {
	...
	// Console 渠道已移除：不再视为有效 Provider，其命名空间也不再被识别。
	if account.ProviderConsole.IsValid() {
		t.Fatal("ProviderConsole should no longer be a valid provider")
	}
	if _, ok := NormalizePublicID(account.ProviderConsole, " Console/grok-4.3 "); ok {
		t.Fatal("removed Console namespace was accepted")
	}
...
func TestExternalPublicIDAndCandidatesSeparateClientAndRouteNames(t *testing.T) {
	...
	want := []string{"Build/grok-4.5", "Web/grok-4.5"}
	...
	// Console 渠道移除后，"Console/" 不再是受支持的来源前缀，
	// 会被当作模型名的一部分参与解析（不会命中任何路由）。
	if got := PublicIDCandidates("Console/grok-4.5"); len(got) != 2 || got[0] != "Build/Console/grok-4.5" || got[1] != "Web/Console/grok-4.5" {
		t.Fatalf("removed Console namespace candidates = %#v", got)
	}
...
func TestNormalizeAndDisplayUpstreamModel(t *testing.T) {
	...
	if _, ok := NormalizeUpstreamModel(account.ProviderBuild, "Console/grok-4.5"); !ok {
		t.Fatal("Console prefix should now be treated as part of the model slug")
	}
	if got := DisplayUpstreamModel(account.ProviderWeb, "grok-chat-fast"); got != "Web/grok-chat-fast" {
