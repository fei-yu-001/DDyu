...
// providers 是本部署实际接入的渠道集合。
// ProviderConsole 常量仍保留：存量数据库的 CHECK 约束与历史数据允许该取值，
// 启动时也会用它清理残留的 Console 模型路由，但不作为可接入渠道对外暴露。
var providers = [...]Provider{ProviderBuild, ProviderWeb}
...
// IsValid 判断 Provider 是否属于当前系统实际接入的渠道。
// ProviderConsole 常量保留用于存量数据兼容与启动时的残留路由清理，
// 但 Console 渠道已移除，不再视为可接入渠道。
func (p Provider) IsValid() bool {
	switch p {
	case ProviderBuild, ProviderWeb:
		return true
	default:
		return false
	}
