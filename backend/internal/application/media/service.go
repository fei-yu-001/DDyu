...
type Service struct {
	...
	// mediaSigner 为作品读取 URL 签发短期签名；须在服务器开始处理请求前注入一次。
	mediaSigner *mediaauth.Signer
}
...
func (s *Service) saveImage(ctx context.Context, data []byte, expiresAt *time.Time, idPrefix string) (mediadomain.Asset, error) {
	...
	asset := mediadomain.Asset{
		...
		ClientKeyID: mediadomain.ClientKeyIDFromContext(ctx),
	}
...
func (s *Service) PublicImageURL(id string) string {
	return s.runtimeConfig().PublicBaseURL + s.SignedMediaPath("images", id)
}

// PublicVideoURL 返回带短期签名的视频作品读取地址。
func (s *Service) PublicVideoURL(id string) string {
	return s.runtimeConfig().PublicBaseURL + s.SignedMediaPath("videos", id)
}

// SetMediaSigner 注入作品读取签名器。必须在服务器开始处理请求前调用一次。
func (s *Service) SetMediaSigner(signer *mediaauth.Signer) {
	s.mediaSigner = signer
}

// SignedMediaPath 返回带短期签名的作品读取相对路径（kind 为 images / videos）。
// 未注入签名器时退化为无签名路径，此时只有携带管理员会话或客户端密钥的请求能读取。
func (s *Service) SignedMediaPath(kind, assetID string) string {
	if s.mediaSigner == nil {
		return "/v1/media/" + kind + "/" + assetID
	}
	return s.mediaSigner.SignPath(kind, assetID)
}
...
// ListClientImages 返回指定客户端密钥名下的图片，实现「各看各的」作品列表。
// clientKeyID 为 0 时返回空结果而不是全量：归属未知必须按失败闭合处理。
func (s *Service) ListClientImages(ctx context.Context, clientKeyID uint64, page, pageSize int, search string) ([]mediadomain.Asset, int64, error) {
	if clientKeyID == 0 {
		return []mediadomain.Asset{}, 0, nil
	}
	return s.assets.ListMediaAssets(ctx, repository.MediaAssetListQuery{
		ClientKeyID: clientKeyID,
		Page:        mediaPageQuery(page, pageSize, search, repository.SortQuery{}),
	})
}
