...
func TestGatewaySSOUnauthorizedMarksInvalidAndSwitchesAccount(t *testing.T) {
	// Console 渠道已移除：SSO 渠道只剩 Grok Web。
	testGatewaySSOFailureMarksInvalidAndSwitchesAccount(t, account.ProviderWeb, http.StatusUnauthorized, `{"error":{"code":"unauthorized","message":"credential rejected"}}`)
}

func TestGatewaySSOBlockedForbiddenMarksInvalidAndSwitchesAccount(t *testing.T) {
	testGatewaySSOFailureMarksInvalidAndSwitchesAccount(t, account.ProviderWeb, http.StatusForbidden, `{"error":{"code":7,"message":"User is blocked [WKE=unauthorized:blocked-user]"}}`)
}
...
func TestGatewayTeamModelRateLimitOnlySkipsMatchingTeam(t *testing.T) {
	...
	for index, seed := range []struct {
		name   string
		teamID string
	}{{"web-team-a-first", "team-a"}, {"web-team-a-second", "team-a"}, {"web-team-b", "team-b"}} {
		credential, _, createErr := accountRepo.UpsertByIdentity(ctx, account.Credential{
			Provider: account.ProviderWeb, AuthType: account.AuthTypeSSO, Name: seed.name, SourceKey: seed.name, TeamID: seed.teamID,
	...
	models := []string{"grok-web-team-rate-limit", "grok-web-team-rate-limit-other"}
	if err := modelRepo.UpsertDiscovered(ctx, account.ProviderWeb, models); err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	for _, credential := range credentials {
	...
	adapter := &teamModelRateLimitWebAdapter{rateLimitedTeam: "team-a"}
	registry := provider.NewRegistry(adapter)
	sticky := memory.NewStickyStore()
	accountService := accountapp.NewService(accountRepo, auditRepo, memory.NewDeviceSessionStore(), sticky, registry, testCipher(t), nil)
	selector := NewSelector(accountRepo, memory.NewConcurrencyLimiter(), sticky, registry, time.Hour, time.Second, time.Minute)
...
func TestSelectConversationRouteRespectsClientKeyAcrossSharedPublicModel(t *testing.T) {
	registry := provider.NewRegistry(&failoverAdapter{}, webStoredResponseAdapter{})
	service := &Service{
	...
	selected, err := service.selectConversationRoute(routes, clientkey.Key{ProviderScope: clientkey.ProviderScopeWeb}, audit.OperationResponses, "/responses", false, nil)
	if err != nil || selected.ID != 15 {
		t.Fatalf("provider-scoped route = %#v, err = %v", selected, err)
	}
	selected, err = service.selectConversationRoute(routes, clientkey.Key{ProviderScope: clientkey.ProviderScopeBuild, AllowedModels: []uint64{10}}, audit.OperationResponses, "/responses", false, nil)
	if err != nil || selected.ID != 10 {
		t.Fatalf("selected route = %#v, err = %v", selected, err)
	}
	_, err = service.selectConversationRoute(routes, clientkey.Key{ProviderScope: clientkey.ProviderScopeBuild, AllowedModels: []uint64{15}}, audit.OperationResponses, "/responses", false, nil)
...
type teamModelRateLimitWebAttempt struct {
	AccountID uint64
	Model     string
}

type teamModelRateLimitWebAdapter struct {
	mu              sync.Mutex
	attempts        []teamModelRateLimitWebAttempt
	rateLimitedTeam string
}

func (a *teamModelRateLimitWebAdapter) Provider() account.Provider {
	return account.ProviderWeb
}
func (a *teamModelRateLimitWebAdapter) Definition() provider.Definition {
	return testConversationDefinition(account.ProviderWeb)
}
func (a *teamModelRateLimitWebAdapter) ForwardResponse(_ context.Context, request provider.ResponseResourceRequest) (*provider.Response, error) {
	a.mu.Lock()
	a.attempts = append(a.attempts, teamModelRateLimitWebAttempt{AccountID: request.Credential.ID, Model: request.Model})
	a.mu.Unlock()
...
func (a *teamModelRateLimitWebAdapter) Attempts() []teamModelRateLimitWebAttempt {
	a.mu.Lock()
	defer a.mu.Unlock()
	return append([]teamModelRateLimitWebAttempt(nil), a.attempts...)
}
...
func TestGatewayBarePermissionDeniedRetainsEgressRetryForWeb(t *testing.T) {
	// Console 渠道已移除：裸 403 的出口重试语义在 Web 渠道上验证。
	for _, providerValue := range []account.Provider{account.ProviderWeb} {
