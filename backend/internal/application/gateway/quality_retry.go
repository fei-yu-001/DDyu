...
type QualityRetryRuntime struct {
	...
	// TreatEncryptedAsThinking promotes a floor-met encrypted_content to
	// plaintext-equivalent thinking evidence so the fast-flush dump heuristics
	// do not misfire on the native /v1/responses pass-through, which has no
	// converter to emit the `: grok2api-reasoning-evidence` marker.
	TreatEncryptedAsThinking bool
}
