...
func TestCreateVideoAppliesRouteConstraintsAfterKeyEligibilityAndBeforeInputIO(t *testing.T) {
	// Console 渠道已移除后，仍需要一个在入队前就会被拒绝的渠道约束场景：
	// Grok Web 仅支持文本生视频，带参考图的请求必须在任何输入 I/O 之前失败。
	routes := []model.Route{
		...
		{ID: 2, PublicID: "shared-video", Provider: account.ProviderWeb, UpstreamModel: "grok-imagine-video", Capability: model.CapabilityVideo},
	}
	...
	service := &Service{
		...
		providers:  provider.NewRegistry(webVideoAdmissionAdapter{}),
	...
	_, err := service.CreateVideo(context.Background(), VideoInput{
		...
		ReferenceURLs: []string{VideoInputFileReference("unused_0")},
		ClientKey: clientkey.Key{
			ProviderScope: clientkey.ProviderScopeWeb,
			AllowedModels: []uint64{2},
		},
	})
	if !errors.Is(err, ErrVideoOperationUnsupported) {
...
type webVideoAdmissionAdapter struct{}

func (webVideoAdmissionAdapter) Provider() account.Provider { return account.ProviderWeb }

func (webVideoAdmissionAdapter) GenerateVideo(context.Context, provider.VideoRequest) (provider.VideoResult, error) {
	return provider.VideoResult{}, errors.New("unexpected Web video generation")
}
