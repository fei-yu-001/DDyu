...
func TestBatchDeleteWithLinkedRemovesPeersAndKeepsUntargetedWeb(t *testing.T) {
	...
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("1", 64), "u1")

	// Console 渠道已移除：仅剩 Web↔Build 一跳关联。
	result, err := service.BatchDeleteWithLinked(ctx, accountdomain.ProviderWeb, []uint64{web.ID}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBatchDeleteWithLinkedWebDeletesBuild(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	repo, service := newLinkedDeleteTestService(t, "svc-linked-delete-web.db")
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("a", 64), "u-web")

	result, err := service.BatchDeleteWithLinked(ctx, accountdomain.ProviderWeb, []uint64{web.ID}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
	if result.Deleted != 2 || result.RootsDeleted != 1 || result.LinkedDeleted != 1 {
		t.Fatalf("result = %#v", result)
	}
	assertAccountMissing(t, repo, web.ID)
	assertAccountMissing(t, repo, build.ID)
...
func TestBatchDeleteWithLinkedMixedBatchOnlyExpandsLinkedRoots(t *testing.T) {
	...
	webLinked, _, _ := seedLinkedTrio(t, repo, strings.Repeat("b", 64), "u-mix")
	...
	// 只有与 Build 关联的根才展开；未关联的 webOnly 仅删自身。
	result, err := service.BatchDeleteWithLinked(ctx, accountdomain.ProviderWeb, []uint64{webLinked.ID, webOnly.ID}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
	if result.Deleted != 3 {
...
func TestPreviewLinkedDeleteCountsWithoutDeleting(t *testing.T) {
	...
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("d", 64), "u-prev")

	res, err := service.PreviewLinkedDelete(ctx, accountdomain.ProviderWeb, []uint64{web.ID}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
	if len(res.RootIDs) != 1 || res.LinkedByProvider[accountdomain.ProviderBuild] != 1 || len(res.FinalIDs) != 2 {
...
func TestBatchDeleteWithLinkedEmptyTargetsDeletesRootOnly(t *testing.T) {
	...
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("e", 64), "u-empty")
...
func TestDeleteWithLinkedRejectsRootFromAnotherProvider(t *testing.T) {
	...
	_, err := service.DeleteWithLinked(ctx, accountdomain.ProviderBuild, web.ID, []accountdomain.Provider{accountdomain.ProviderWeb})
