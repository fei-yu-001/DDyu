...
// ReconcileRateLimit 根据额度模式核实 429。Web 周池以上游快照为准。
// 历史上 Console 的 resource-exhausted 可能表示瞬时 RPS/RPM 限流，需要先查询
// /usage 再判断；Console 渠道移除后仅保留 Web 快照路径。
...
func (s *Service) QueueQuotaRefresh(id uint64, mode string) {
	...
	if id == 0 || (mode != "weekly" && mode != accountdomain.QuotaGroupWebImagine && !isWebChatQuotaMode(mode)) {
		return
	}
...
func (s *Service) runQuotaRefresh(parent context.Context, request quotaRefreshRequest) {
	for {
		...
		if windows, err := s.accounts.GetQuotaWindows(ctx, []uint64{request.accountID}); err == nil {
			...
			if request.mode != accountdomain.QuotaGroupWebImagine {
...
// quotaWindowControlsRouting 报告该渠道是否以额度窗口参与路由。
// 历史上 Console 固定读取 /usage 快照；Console 渠道移除后，保留的 Build 与 Web
// 均以额度窗口参与路由。
func quotaWindowControlsRouting(accountdomain.Provider, string) bool {
	return true
}
