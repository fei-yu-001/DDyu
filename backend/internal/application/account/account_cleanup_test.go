...
func TestCleanupAccountsWithLinkedTargets(t *testing.T) {
	...
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("7", 64), "u-cleanup")
	...
	// Console 渠道已移除：清理只级联到 Web↔Build 关联。
	result, err := service.CleanupAccounts(ctx, accountdomain.ProviderWeb, []CleanupStatus{CleanupStatusReauthRequired}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
	if result.Deleted != 2 || result.RootsDeleted != 1 || result.LinkedDeleted != 1 || result.Skipped != 0 {
		t.Fatalf("result = %#v", result)
	}
...
func TestPreviewCleanupCountsWithoutDeleting(t *testing.T) {
	...
	web, build, _ := seedLinkedTrio(t, repo, strings.Repeat("9", 64), "u-preview")
	web.Enabled = false
	if _, err := repo.Update(ctx, web); err != nil {
		t.Fatal(err)
	}

	preview, err := service.PreviewCleanup(ctx, accountdomain.ProviderWeb, []CleanupStatus{CleanupStatusDisabled, CleanupStatusCooldown}, []accountdomain.Provider{accountdomain.ProviderBuild})
	if err != nil {
		t.Fatal(err)
	}
	if preview.RootsByStatus["disabled"] != 1 || preview.RootsByStatus["cooldown"] != 0 || preview.RootCount != 1 || preview.Total != 2 {
