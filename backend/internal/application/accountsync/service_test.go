...
func TestSyncAccountIgnoresBestEffortSSOIdentityFailure(t *testing.T) {
	// Console 渠道已移除：仅 Web 走 SSO 身份同步。
	reader := &identityAccountReaderStub{accountReaderStub: accountReaderStub{provider: accountdomain.ProviderWeb}, err: errors.New("session unavailable")}
	quota := &quotaStub{}
	models := &modelStub{hasSnapshot: true}
...
func TestSyncAccountStopsAfterSSOIdentityUnauthorized(t *testing.T) {
	reader := &identityAccountReaderStub{accountReaderStub: accountReaderStub{provider: accountdomain.ProviderWeb}, err: provider.ErrUnauthorized}
	quota := &quotaStub{}
	models := &modelStub{hasSnapshot: true}
	service := NewService(slog.Default(), reader, &billingStub{}, quota, models)
