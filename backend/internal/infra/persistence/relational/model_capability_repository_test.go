...
func TestModelRouteGroupsKeepCapabilitiesCompleteAcrossStatusFilters(t *testing.T) {
	...
	const publicID = "grouped-web-image"
	if err := models.UpsertRoutes(ctx, []model.Route{
		{PublicID: publicID, Provider: account.ProviderWeb, UpstreamModel: "grok-imagine-image", Capability: model.CapabilityImage, Origin: model.OriginCatalog, Enabled: true},
		{PublicID: publicID, Provider: account.ProviderWeb, UpstreamModel: "grok-imagine-image", Capability: model.CapabilityImageEdit, Origin: model.OriginCatalog, Enabled: true},
	}); err != nil {
		t.Fatal(err)
	}
	...
	for _, capability := range []model.Capability{model.CapabilityImage, model.CapabilityImageEdit} {
		if _, err := models.Create(ctx, model.Route{
			PublicID: "manual-group-target", Provider: account.ProviderWeb, UpstreamModel: "grok-imagine-image",
...
func TestPublicModelNameResolvesAcrossAvailableProviders(t *testing.T) {
	...
	console, _, err := accounts.UpsertByIdentity(ctx, account.Credential{
		Provider: account.ProviderWeb, Name: "web", SourceKey: "shared-web",
		EncryptedAccessToken: testEncryptedToken, Enabled: true, AuthStatus: account.AuthStatusActive,
	})
	if err != nil {
		t.Fatal(err)
	...
	if err != nil || len(routes) != 2 || routes[0].Provider != account.ProviderBuild || routes[1].Provider != account.ProviderWeb {
		t.Fatalf("shared routes = %#v, err = %v", routes, err)
	}
	explicit, err := models.GetByPublicIDCandidates(ctx, "Web/grok-shared")
	if err != nil || len(explicit) != 1 || explicit[0].Provider != account.ProviderWeb {
	...
	if err != nil || route.Provider != account.ProviderWeb {
