...
func (r *MediaAssetRepository) ListMediaAssets(ctx context.Context, input repository.MediaAssetListQuery) ([]media.Asset, int64, error) {
	...
	if input.ClientKeyID != 0 {
		query = query.Where("client_key_id = ?", input.ClientKeyID)
	}
	...
	if err := query.Select("id", "kind", "mime_type", "size_bytes", "sha256", "client_key_id", "created_at").Order("created_at DESC, id DESC").Offset(input.Page.Offset).Limit(input.Page.Limit).Find(&rows).Error; err != nil {
		return nil, 0, err
	}
