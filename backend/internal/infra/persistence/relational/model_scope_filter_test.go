...
func assertModelListFiltersByClientKeyProviderAndTierScope(t *testing.T, database *Database) {
	...
	if len(free) != 2 || !free["Build/scope-free"] || !free["Web/scope-web-free"] || free["Build/scope-super"] || free["Web/scope-web-super"] {
		t.Fatalf("free scope models = %#v", free)
	}
	super := list([]string{"super"}, nil)
	if len(super) != 2 || !super["Build/scope-super"] || !super["Web/scope-web-super"] || super["Build/scope-free"] || super["Web/scope-web-free"] {
	...
	// 停用后剩余渠道没有 super 分层账号，因此不应再对外声明任何 super 路由。
	enabledSuper := enabledForScope([]string{"super"}, nil)
	if len(enabledSuper) != 0 {
