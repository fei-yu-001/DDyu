...
func (a *Adapter) GenerateVideo(ctx context.Context, request provider.VideoRequest) (provider.VideoResult, error) {
	...
	// TEMP(experiment): DDYU_VIDEO_DUMP 将上游原始响应落盘用于定位，验证后移除。
	if dumpPath := os.Getenv("DDYU_VIDEO_DUMP"); dumpPath != "" {
...
func videoCreatePayload(prompt, ratio, resolution string, seconds int) map[string]any {
	return videoCreatePayloadVariant(prompt, ratio, resolution, seconds, os.Getenv("DDYU_VIDEO_VARIANT"))
}

// videoCreatePayloadVariant 为定位「上游不采信提示词」的根因而临时引入的开关。
// 验证完成后必须移除该开关并固化结论形态。
func videoCreatePayloadVariant(prompt, ratio, resolution string, seconds int, variant string) map[string]any {
	...
	cleanMessage := func() { payload["message"] = prompt }
	dropMeta := func() { delete(payload, "responseMetadata") }
	dropKind := func() { delete(payload, "kind") }
	switch variant {
	case "v1":
		cleanMessage()
	case "v2":
	...
	case "v7":
		...
		dropKind()
	case "v8":
		// 只用 message 驱动，完全不带 mediaGenInput（模拟纯对话式 Imagine）
		dropMeta()
		dropKind()
		cleanMessage()
		delete(payload, "mediaGenInput")
	}
	return payload
}
