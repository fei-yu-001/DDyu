...
type FrontendConfig struct {
	...
	BlogStaticPath           string `yaml:"blogStaticPath"`
}
...
type QualityGuardRequestRetryConfig struct {
	...
	// TreatEncryptedAsThinking lets a floor-met upstream encrypted_content
	// count as plaintext-equivalent thinking evidence. The native
	// /v1/responses pass-through has no protocol converter to emit the
	// `: grok2api-reasoning-evidence` marker used by the Chat/Messages path,
	// so without this the fast-flush dump heuristics misfire on healthy Build
	// responses. Short stubs below the ciphertext floor stay plaintext-less
	// and are still withheld as missing thinking.
	TreatEncryptedAsThinking bool `yaml:"treatEncryptedAsThinking"`
}
...
func resolveRelativePaths(cfg *Config, configPath string) error {
	...
	blogPath := strings.TrimSpace(cfg.Frontend.BlogStaticPath)
	if blogPath != "" && !filepath.IsAbs(blogPath) {
		cfg.Frontend.BlogStaticPath = filepath.Clean(filepath.Join(baseDir, blogPath))
	}
	return nil
}
...
func defaultConfig() Config {
	return Config{
		...
		Frontend: FrontendConfig{PublicAPIBaseURL: DefaultPublicAPIBaseURL, StaticPath: "./frontend/dist", BlogStaticPath: "./static/blog"},
