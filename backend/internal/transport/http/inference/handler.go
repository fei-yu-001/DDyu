...
type Handler struct {
	...
	// mediaReadURL 生成带短期签名的作品读取路径（kind 为 images / videos）。
	mediaReadURL func(kind, assetID string) string
}
...
// SetMediaReadURLResolver 注入带短期签名的作品读取路径生成器，使视频直链能在
// 浏览器/播放器里直接打开（<video> 无法携带请求头，只能把凭证放进 URL）。
func (h *Handler) SetMediaReadURLResolver(resolve func(kind, assetID string) string) *Handler {
	h.mediaReadURL = resolve
	return h
}
...
type modelListItem struct {
	...
	// Capability 供前端按能力（对话/生图/图编/视频）分组；上游渠道 Provider 仍只在
	// 服务端内部使用，不回显给客户端。
	Capability modeldomain.Capability `json:"capability"`
...
// videoPlaybackURL prefers the stored asset served by the media route, so the returned
// link opens directly in browsers and players. The media route now requires either an
// administrator session or a short-lived signature, and <video> cannot carry headers,
// so the resolver injects a signed path. /v1/videos/{id}/content stays the fallback for
// jobs without a stored asset (it needs the client API key).
func (h *Handler) videoPlaybackURL(job mediadomain.Job) string {
	if assetID := strings.TrimSpace(job.ResultAssetID); assetID != "" {
		...
		if h.mediaReadURL != nil {
			path = h.mediaReadURL("videos", assetID)
		}
