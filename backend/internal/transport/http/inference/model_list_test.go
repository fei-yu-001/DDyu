...
func TestNewModelListItemsDeduplicatesSharedPublicName(t *testing.T) {
	...
	items := newModelListItems([]modeldomain.Route{
		...
		{PublicID: "Web/grok-shared", Provider: account.ProviderWeb, CreatedAt: now.Add(time.Second)},
		...
		{PublicID: "Web/grok-imagine-image", Provider: account.ProviderWeb, Capability: modeldomain.CapabilityImageEdit, CreatedAt: now.Add(time.Second)},
	})
...
func TestAppendReasoningModelAliasesUsesRealSupportedLevels(t *testing.T) {
	...
	for _, want := range []string{"grok-4.5", "grok-4.5-low", "grok-4.5-medium", "grok-4.5-high", "grok-4.6", "grok-4.6-low", "grok-4.6-medium", "grok-4.6-high", "grok-4.6-xhigh", "grok-build-0.1"} {
		if !ids[want] {
			t.Fatalf("missing model %q in %#v", want, expanded)
		}
	}
	for _, reject := range []string{
		"grok-4.5-none", "grok-4.5-xhigh", "grok-4.5-max", "grok-build-0.1-none",
	} {
		if ids[reject] {
...
func TestAppendReasoningModelAliasesIncludesBuildGrok46XHigh(t *testing.T) {
	...
	for _, want := range []string{"grok-4.6-low", "grok-4.6-medium", "grok-4.6-high", "grok-4.6-xhigh"} {
		if !ids[want] {
			t.Fatalf("missing alias %q in %#v", want, expanded)
		}
	}
	if ids["grok-4.6-max"] {
...
// Console 渠道已移除，其专属的「固定推理等级」用例（原
// TestCodexCatalogMarksConsoleGrok420AsFixedReasoning）随之删除：该分支已不存在。
