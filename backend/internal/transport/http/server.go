...
type Dependencies struct {
	...
	// MediaReadSigner 为作品读取端点签发短期签名；
	// MediaReadURLResolver 生成带签名的作品读取路径，供视频直链复用同一套签名。
	MediaReadSigner      *mediaauth.Signer
	MediaReadURLResolver func(kind, assetID string) string
}
...
func New(deps Dependencies) *gin.Engine {
	...
	// 上游回调端点（票据即授权）保持公开；作品读取需要管理员会话或客户端密钥。
	mediaHandler.RegisterPublic(router)
	mediaHandler.RegisterRead(router, middleware.MediaReadAuth(deps.AdminAuth, deps.ClientKeys, deps.MediaReadSigner))
	...
	if deps.MediaReadURLResolver != nil {
		inferenceHandler.SetMediaReadURLResolver(deps.MediaReadURLResolver)
	}
	inferenceHandler.Register(v1)
	// 面向客户端密钥的作品列表：只返回该密钥名下的作品。
	mediaHandler.RegisterClient(v1)
	// 客户端密钥可用的临时输入端点（图编首帧图、视频参考图/源视频的上传与导入）。
	mediaHandler.RegisterClientInputs(v1)
	registerFrontend(router, deps.AdminAuth, deps.FrontendStaticPath, deps.BlogStaticPath)
	return router
}
