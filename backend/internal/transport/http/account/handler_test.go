...
func TestParseLinkedDeleteTargets(t *testing.T) {
	targets, err := parseLinkedDeleteTargets([]string{"grok_build", "grok_web", "grok_build"})
	if err != nil || len(targets) != 2 || targets[0] != accountdomain.ProviderBuild || targets[1] != accountdomain.ProviderWeb {
		t.Fatalf("targets=%v err=%v", targets, err)
	}
	if _, err := parseLinkedDeleteTargets([]string{"nope"}); err == nil {
		t.Fatal("expected invalid target")
	}
	// Console 渠道已移除：历史取值必须被拒绝，避免残留调用被静默接受。
	if _, err := parseLinkedDeleteTargets([]string{"grok_console"}); err == nil {
		t.Fatal("expected removed console provider to be rejected")
	}
