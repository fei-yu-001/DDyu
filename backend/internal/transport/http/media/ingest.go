...
func (h *Handler) writeInputAsset(c *gin.Context, asset mediadomain.Asset) {
	...
	// 同时返回带短期签名的读取地址：图编（/v1/images/edits）只接受 image.url，
	// 且该 URL 会被上游抓取，因此必须是无鉴权头也能读取的签名地址。
	assetURL := h.service.PublicImageURL(asset.ID)
	if asset.Kind == "video" {
		assetURL = h.service.PublicVideoURL(asset.ID)
	}
