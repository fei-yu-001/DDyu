...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	// 本部署只保留 Build（对话 + 凭据刷新）与 Web（生图/图编/文生视频）两个渠道。
	// Console 渠道已移除，其独有的语音能力随之下线。
	...
	// Grok Console 渠道已移除：用空目录覆盖，清掉存量库里已不可路由的 Console 模型。
	// 手动创建的路由不会被覆盖，站主仍可自行保留需要的别名。
	if err := modelRepo.ReplaceProviderRoutes(ctx, account.ProviderConsole, nil); err != nil {
		if runtimeStore != nil {
			_ = runtimeStore.Close()
		}
		database.Close()
		return nil, fmt.Errorf("清理 Grok Console 模型目录: %w", err)
	}
	...
	mediaReadSigner, err := mediaauth.NewSigner(cfg.Secrets.JWTSecret, 0)
	if err != nil {
		if runtimeStore != nil {
			_ = runtimeStore.Close()
		}
		database.Close()
		return nil, fmt.Errorf("初始化作品读取签名: %w", err)
	}
	mediaService.SetMediaSigner(mediaReadSigner)
	router := httpserver.New(httpserver.Dependencies{Logger: logger, RequestTimeout: cfg.Server.RequestTimeout.Value(), MaxBodyBytes: cfg.Server.MaxBodyBytes, TrustedProxies: cfg.Server.TrustedProxies, ConcurrencyGate: inferenceConcurrency, SecureCookies: cfg.Auth.SecureCookies, SwaggerEnabled: cfg.Server.SwaggerEnabled, PublicAPIBaseURL: cfg.Frontend.EffectivePublicAPIBaseURL(), FrontendStaticPath: cfg.Frontend.StaticPath, BlogStaticPath: cfg.Frontend.BlogStaticPath, Readiness: readiness, TrafficReady: startup.acceptsTraffic, AdminAuth: adminService, Accounts: accountService, AccountSync: accountSyncService, Models: modelService, ClientKeys: clientKeyService, Audits: auditService, Dashboard: dashboardService, Gateway: gatewayService, Media: mediaService, Settings: settingsService, Egress: egressService, QualityGuardStatePath: qualityGuardPath("state.json"), QualityGuardConfigPath: qualityGuardPath("runtime-config.json"), QualityGuardToken: qualityGuardToken, QualityGuardProbe: qualityGuardProbe, Updates: updateService, MediaReadSigner: mediaReadSigner, MediaReadURLResolver: mediaService.SignedMediaPath})
...
func qualityRetryRuntime(value config.QualityGuardRequestRetryConfig) gateway.QualityRetryRuntime {
	return gateway.QualityRetryRuntime{
		...
		TreatEncryptedAsThinking:         value.TreatEncryptedAsThinking,
	}
}
