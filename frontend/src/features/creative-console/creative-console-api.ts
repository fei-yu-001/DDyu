  ...
  const payload = await publicApiRequest(
    ...
    "/images/edits",
    {
      ...
      body: {
        model: input.model,
        prompt: input.prompt,
        // 网关当前只接受 image.url（file_id 会返回 unsupported_parameter），
        // 且该地址由上游抓取，因此必须是可公开读取的地址。
...
export type ClientInputAsset = {
  fileId: string;
  kind: string;
  mimeType: string;
  sizeBytes: number;
  expiresAt: string;
  /** 带短期签名的读取地址；图编等需要上游直接抓取图片的场景会用到。 */
  url: string;
};

function readInputAsset(payload: unknown): ClientInputAsset {
  const value = isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
  if (!isRecord(value) || typeof value.fileId !== "string") {
    throw new CreativeApiError(200, "The media input response was invalid", "invalid_response");
  }
  return {
    fileId: value.fileId,
...
// 上传本地图片/视频到带 TTL 的临时输入区。走客户端密钥鉴权的 /v1 端点，
// 因此创意工坊无需管理员登录态；不要手动设置 Content-Type，浏览器要带 multipart 边界。
export async function uploadClientInput(apiKey: string, file: File): Promise<ClientInputAsset> {
  const form = new FormData();
  form.append("file", file, file.name);
  const response = await fetch("/v1/media/inputs/upload", {
    method: "POST",
  ...
  if (!response.ok) {
    const error = readError(parseJSON(text));
    throw new CreativeApiError(response.status, error.message ?? (text.trim() || response.statusText || `HTTP ${response.status}`), error.code);
  }
  return readInputAsset(parseJSON(text));
}

// 从远端 URL 导入一张图片到临时输入区（SSRF 防护与 MIME 校验在服务端完成）。
export async function importClientInput(apiKey: string, url: string): Promise<ClientInputAsset> {
  const payload = await publicApiRequest(apiKey, "/media/inputs/import", { method: "POST", body: { url } });
  return readInputAsset(payload);
}
...
// listClientModels 列出当前客户端密钥可用的模型。走 /v1/models（OpenAI 兼容形状），
// 不依赖管理端接口，因此创意工坊不需要管理员登录态。
export async function listClientModels(input: { apiKey: string; signal?: AbortSignal }): Promise<ClientModel[]> {
  const payload = await publicApiRequest(input.apiKey, "/models", { method: "GET", signal: input.signal });
  if (!isRecord(payload) || !Array.isArray(payload.data)) {
    throw new CreativeApiError(200, "The model list response was invalid", "invalid_response");
  }
  return payload.data.flatMap((item) => {
    if (!isRecord(item) || typeof item.id !== "string") return [];
    return [{ id: item.id, capability: typeof item.capability === "string" ? item.capability : "" }];
  });
}
...
// Console 语音链路（TTS / STT / 音色列表）已随渠道裁剪下线，对应客户端封装一并移除：
// /v1/tts、/v1/stt、/v1/tts/voices 在本部署中不再存在，保留调用只会得到 404。
