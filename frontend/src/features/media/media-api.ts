...
// 作品读取现在需要凭证（管理员会话或短期签名），列表接口返回的 url 已带签名参数，
// 因此必须保留它的 query。同源时退化为相对路径，避免把主机名写死进页面
// （换域名或经 Cloudflare Tunnel 访问时仍然可用）。
export function mediaReadURL(url: string, fallbackPath: string): string {
