...
import { deleteVideos, getVideoStats, listVideos, mediaReadURL } from "@/features/media/media-api";
    ...
    <div className="space-y-5">
      ...
      <Dialog open={Boolean(previewing)} onOpenChange={(open) => !open && setPreviewing(null)}>
        <DialogContent className="max-h-[calc(100vh-2rem)] max-w-4xl overflow-hidden">
          ...
          {previewing?.assetId ? (
            <VideoPreview key={previewing.assetId} assetId={previewing.assetId} url={previewing.url} />
          ) : null}
        </DialogContent>
      </Dialog>
    </div>
...
function VideoPreview({ assetId, url }: { assetId: string; url: string }) {
  ...
  return (
    <div className="relative flex min-h-56 w-full items-center justify-center overflow-hidden rounded-lg bg-black sm:min-h-80">
      <video
        ...
        src={mediaReadURL(url, videoAssetURL(assetId))}
        controls
        playsInline
        preload="auto"
