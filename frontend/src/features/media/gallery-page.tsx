...
import { deleteImages, getImageStats, listImages, mediaReadURL } from "@/features/media/media-api";
...
function ImageCard({ image, locale, selectionMode, selected, onSelectedChange }: { image: MediaAssetDTO; locale: string; selectionMode: boolean; selected: boolean; onSelectedChange: (checked: boolean) => void }) {
  ...
  // 作品读取需要凭证：列表接口返回的 url 带短期签名，必须沿用它的 query；
  // 缺失时退回同源相对路径（此时只有管理员会话能读）。
  const imageURL = mediaReadURL(image.url, `/v1/media/images/${encodeURIComponent(image.id)}`);
  return (
